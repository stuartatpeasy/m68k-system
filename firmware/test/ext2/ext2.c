...
u32 ext2_read_block(const ext2_filesystem_t *fs, ku32 block, u8 **ppbuf)
{
	...
	if(*ppbuf == NULL)
	{
		buf = malloc(1024 << fs->sblk->s_log_block_size);
		if(buf == NULL)
		{
			return ENOMEM;
		}
...
u32 ext2_init_filesystem(ext2_filesystem_t **ppfs)
{
	...
	*/
	...
	buf = malloc(sizeof(ext2_superblock_t));
	if(!buf)
		return ENOMEM;
...
#ifdef DEBUG
void ext2_dump_inode(const ext2_inode_t inode)
{
	u32 i;

	puts("# Inode dump");
	printf("# i_mode: 0x%04x/0%03o uid=%u gid=%u\n", inode.i_mode & 0xfe00, 
												inode.i_mode & 0x1ff, inode.i_uid, inode.i_gid);

	printf("#\n# data blocks: ");
	for(i = 0; i < 15; i++)
		printf("%9u ", inode.i_block[i]);
	puts("");
}

#else
#define ext2_dump_inode(x)
#endif
